...
Droid review is the default when no engine is set. Codex usually delivers the best review results and can be selected with `--engine codex`.
...
- supports `--engine codex`, `claude`, `droid`, and `opencode`; default is `AUTOREVIEW_ENGINE` or `droid`; use `--engine codex` when Codex is available
