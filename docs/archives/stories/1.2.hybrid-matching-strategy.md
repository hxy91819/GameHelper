...
6. 如果模糊匹配（例如 `Fuzz.Ratio` > 80）成功，则匹配该配置。
   > ⚠️ **需要增强（Story 1.7）**：固定 80 分阈值在短文件名（2-4 字符）场景下会产生误匹配（如 `reg.exe` → `RE.exe`，score 92）。Story 1.7 引入三层安全边界（系统路径黑名单、长度感知动态阈值、路径相关性验证）取代单一 `> 80` 判定。详见 `docs/stories/1.7.enhance-fuzzy-matching-safety.md`。
