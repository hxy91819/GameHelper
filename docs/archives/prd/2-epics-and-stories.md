...
> 当前状态：史诗已完成，实施材料与 QA 记录已归档到 `docs/archives/`。
...
| Story | 主题 | 当前状态 | 备注 |
| --- | --- | --- | --- |
| 1.1 | 数据模型与配置加载 | 已完成 | `DataKey` / `ExecutablePath` 模型已上线 |
| 1.2 | 混合匹配策略 | 已完成 | 安全边界由 1.7 补强 |
| 1.3 | 拖放和交互式配置 | 已完成 | 已支持 `.exe` / `.lnk` / `.url` |
| 1.4 | 数据关联 | 已完成 | `playtime.csv` 与 `stats` 已按 `DataKey` 工作 |
| 1.5 | 历史数据迁移工具 | 已完成 | 当前 CLI 使用 `migrate` / `migrate-config` |
| 1.6 | ETW 默认监听 | 已完成 | 默认 ETW，自动回退 WMI |
| 1.7 | 模糊匹配安全加固 | 已完成 | 动态阈值、黑名单、路径校验已落地 |
...
* **验收标准：**
    ...
    6.  如果模糊匹配成功，则匹配该配置；最终实现已由 Story 1.7 补充动态阈值、黑名单和路径验证约束。
...
**故事 1.7：增强 L2 模糊匹配的安全性（修复 Story 1.2 缺陷）**
* **作为** 系统管理员,
* **我想要** L2 模糊匹配具有更严格的安全边界和验证机制,
* **以便** 避免将无关的系统工具（如 `reg.exe`、`rg.exe`）错误匹配为游戏，确保游戏时长统计的准确性。
* **背景：** Story 1.2 的实现存在严重缺陷，仅基于文件名相似度进行匹配，导致系统工具被错误识别为游戏。
* **验收标准：**
    1.  L2 模糊匹配成功后，必须验证进程路径与配置路径的目录相关性（如果配置有路径）。
    2.  必须实现动态阈值策略：
        - 文件名长度 2-4 字符：阈值 ≥ 95
        - 文件名长度 5-8 字符：阈值 ≥ 90
        - 文件名长度 9+ 字符：阈值 ≥ 80
    3.  必须拒绝匹配来自系统目录的进程（`C:\Windows\System32\`、`C:\Windows\SysWOW64\`、`C:\Windows\`）。
    4.  必须记录详细的结构化日志（搜索名称、候选配置、阈值、分数、路径验证结果、最终决策）。
    5.  必须添加单元测试验证动态阈值、路径验证和黑名单检查。
    6.  必须添加回归测试验证 `reg.exe` 和 `rg.exe` 不再匹配 `RE.exe`。
    7.  对于仅有 `ExecutableName` 的旧配置，路径验证必须跳过（保持兼容性）。

## 历史记录

- 故事卡、QA Gate、变更提案和复盘材料已迁移到 `docs/archives/`。
