...
- 报告不含"生成时刻"，同一自然日内内容指纹稳定；但"最近 7 天/本周/本月"窗口随自然日滑动，跨天时即使数据未变也会产生一次窗口更新的提交——这与"每天最多推一次"的默认节奏一致，报告的更新时间看仓库提交历史即可。
...
- **内容去重**：同一自然日内内容与上次一致时跳过上传，不产生空提交（跨天时报告窗口自然滑动，见上文）。
