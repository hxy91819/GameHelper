...
- 按游戏聚合最近 14 个自然日（含今天）的游玩记录，显示次数、总时长和最近游玩时间，按总时长排序；同一天多次启动同一游戏不再重复占行。
- 面板下方附最近 14 天的每日游玩时长趋势迷你图（4 行纵向柱状：每天一列，自底向上按当日时长占峰值的比例填充块字符，峰值日占满 4 行；底行零值天以灰点占位，下方标注起止日期；按会话结束时间的本地日期归属）。
