...
2.  **回退匹配 (L2，含安全边界):** 对于仅配置了 `ExecutableName` 的条目，使用文件元数据 (`FileVersionInfo.ProductName`) 和模糊匹配 (FuzzySharp) 进行回退。**Story 1.7 引入三层安全边界**以避免短文件名误匹配（如 `reg.exe` → `RE.exe`）：
    - **系统路径黑名单**：拒绝来自 `C:\Windows\System32\`、`C:\Windows\SysWOW64\`、`C:\Windows\` 的进程。
    - **长度感知动态阈值**：基于 `ExecutableName` 去扩展名后的长度选择阈值（≤4 字符要求 ≥95，5-8 字符 ≥90，>8 字符 ≥80）。
    - **路径相关性验证**：当配置存在 `ExecutablePath` 时，要求进程路径与配置路径同目录树（旧配置无 `ExecutablePath` 时跳过此项）。

详见 `docs/stories/1.7.enhance-fuzzy-matching-safety.md` 与 `GameHelper.Tests/FuzzyMatchingSafetyTests.cs`。
